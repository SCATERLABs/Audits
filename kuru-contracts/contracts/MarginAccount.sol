...
contract MarginAccount is IMarginAccount, ERC2771Context, Ownable, Initializable, UUPSUpgradeable {
    ...
    function initialize(address _owner, address _router, address _feeCollector, address _trustedForwarder)
        ...
        initializer
    {
        //check zero address once check it for all addresses
        // require(_owner != address(0), MarginAccountErrors.ZeroAddressNotAllowed());
        // require(_router != address(0), MarginAccountErrors.ZeroAddressNotAllowed());
        // require(_feeCollector != address(0), MarginAccountErrors.ZeroAddressNotAllowed());
        // require(_trustedForwarder != address(0), MarginAccountErrors.ZeroAddressNotAllowed());
     ...
     */
    function 
    (bytes calldata _encodedData) external protocolActive {
        require(verifiedMarket[msg.sender], MarginAccountErrors.OnlyVerifiedMarketsAllowed());
    ...
    function deposit(address _user, address _token, uint256 _amount) external payable protocolActive {
        ...
        if (_token == NATIVE) {
            ...
            //@nk here dont have safeTransfer function to send eth to the user
        } else {
    ...
    function withdraw(uint256 _amount, address _token) external protocolActive {
        //@nk check effects and interactions pattern is followed? ,i think it is followed
        ...
        if (_token == NATIVE) {
            //@nk safeTransferETH  function call is used to transfer native tokens,how does it work internally check it
...
/**
issues to be noted:
1.If anyone  can send eth directly to the contract then it will be locked there forever? is that an issue?
*/

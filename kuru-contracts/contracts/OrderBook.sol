...
contract OrderBook is Initializable, UUPSUpgradeable, ERC2771Context, AbstractAMM {
    ...
    function initialize(
        ...
        uint96 _sizePrecision,//what is size precision? 
...
// @audit but here remaining consumed funds is added to the consumed then alll the funds is added order to the margin account ?is this correct? yes or no? 
        _consumedFunds += _quoteAmountRoundedUp(_price, _remainingSize);
//@CHECK Vulnerability if: rounding is too aggressive and the excess accumulates (griefing or draining takers). 
        ...
        nonReentrant
    {
        ...
        uint40 _orderId = s_orderIdCounter + 1;
        s_orderIdCounter = _orderId;
        //current id becomes previous id for next order
        uint40 _prevOrderId = OrderLinkedList.insertAtTail(s_buyPricePoints[_price], _orderId);
    ...
    function _consumeFunds(address _consumableAsset, uint256 _amount, address _userAddress) internal {
        //here send the amount to the margin account
        ...
        returns (uint256)
    {
        ...
        if (_type == OrderBookType.NATIVE_IN_QUOTE && !_isMargin && (msg.sender != address(0))) {
            ...
            _isMargin//isMargin is used to determine if user wants to use margin account balance or transfer from wallet
        ...
        returns (uint96, uint96)
    {
        ...
        //getting best ask price in vault price precision
        ...
        while (_sizeToBeFilled > 0 && _bestAsk <= _limitPrice && _bestAsk != 0) {
            ...
            if (_isVaultFill) {
                ...
                // If user set the limit price below the best ask,we fill at limit Price+1 tick
            ...
            if (_isVaultFill) {
                //if the isVaultFill is true,it means user is buying from vault
...
//end loop
        ...
        }
        {
            ...
            //convert token credit to base asset like ETh ,
            uint256 _tokenCredit = (sizeToCreditTaker * _baseDecimalMultiplier) / _sizePrecision;
            if (sizeToCreditViaVault > 0) {
                ///debit amount from the kuruAmmVault  to the user
                marginAccount.debitUser(
                    kuruAmmVault, baseAsset, (sizeToCreditViaVault * _baseDecimalMultiplier) / _sizePrecision
                );
                ///// marginAccount.debitUser(_userAddress, _consumableAsset, _amount);
            }
            //tokencredit-=sizeToCreditViaVault*baseDecimalMultiplier/sizePrecision;
            //@audit after the token credit calculation the token credit send amount to the kuruAmmvault but not reduce the token credit by that amount is this an issue ? or not ? answer not 
        ...
        returns (uint96, uint256)
    {
        ...
        while (_sizeToBeFilledLeft > 0 && _bestBid >= _limitPrice && _bestBid != type(uint256).max) {
            ...
            if (_isVaultFill) {//using amm vault to place sell orders 
                ...
                //quoteCreditTaker+=_sizeToBeFilledBefore-_sizeToBeFilledLeft;
            } else {
                ...
                quoteCreditTaker += toU96(//why here not using mulDivUp? explain  is there any issue?  answer no issue why ? because we are crediting the user not debiting so its better to round down
                    FixedPointMathLib.mulDiv(
    ...
    function _fillSizeForPrice(
        ...
        //its very needed to check if the price point is empty and remove it from the tree

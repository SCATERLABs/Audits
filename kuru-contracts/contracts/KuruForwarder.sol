...
contract KuruForwarder is EIP712, Initializable, Ownable, UUPSUpgradeable {
    ...
    function verify(ForwardRequest calldata req, bytes calldata signature) public view returns (bool) {
        ...
        //first  encode  then i will convert into bytes32 and then hash it using _hashTypeData
...
//i think this function have an issuee
        ...
        returns (bool)
    {
        ...
        //if not initialized  _nonces[req.from] then it will be 0 or not ?answer is yes it will be zero
        return req.nonce >= _nonces[req.from] && signer == req.from;
    }



//  @nk-audit if i send eth more than req.value will it be accepted ? yes it will be accepted because msg.value >= req.value but remaining eth will be stuck in the contract? is this right? or wrong? yes remaining eth will be stuck in the contract   
        ...
        returns (bytes memory)
    {
        ...
        //@nk check margin account is valid contract address or not?
    ...
    //@nk check market is valid contract address or not?
        (bool success, bytes memory returndata) =
...
//@audit -nk issue is here ,it is require(
...
/**
If any noteworthy vulnerabilities are found in this contract,below document them.
1. if i send eth more than req.value will it be accepted ? yes it will be accepted because msg.value >= req.value but remaining eth will be stuck in the contract? is this right? or wrong? yes remaining eth will be stuck in the contract
 2.
 */

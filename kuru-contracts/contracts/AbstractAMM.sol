...
abstract contract AbstractAMM is IOrderBook, ReentrancyGuardTransient {
    ...
    {
        //foucs on this only not anything
        ...
        while (_price < _breakPoint && _quoteInput > 0) {
            ...
            if (_quoteNeededForFill > _quoteInput) {
                //@check what is the use of this  below line
        ...
        if (_price != vaultBestAsk) {
            //here update the prices and sizes for vault
            vaultBestAsk = _price;
            ...
            vaultBidOrderSize = toU96(//how much vault is read to buy  this is called vault bid order size
                FixedPointMathLib.mulDiv(

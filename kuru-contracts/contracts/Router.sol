...
contract Router is IRouter, Ownable, Initializable, UUPSUpgradeable {
    ...
    mapping(address => MarketParams) public verifiedMarket;//verified markets to check that the market is deployed by the router
     ...
     */
     ...
     // here router  is iniialized only when the all the other contracts are deployed? Is this correct?
    function initialize(
        address _owner,
    ...
    function computeVaultAddress(address _marketAddress, address oldImplementation, bool old)
        ...
        returns (address)
    {
        //@audit this is an issue because here not used proxy address as a salt they are used market address which creates a ambiguity
     ...
     */
     ...
     //@audit check once anyToAnySwap function properly
    function anyToAnySwap(
        ...
        if (_nativeSend[0] == false) {
            // if the first nativesend is false then debit token should not be native then transfer the funds to the router contract
        ...
        for (uint256 i = 0; i < _marketAddresses.length; i++) {
            ...
            //@nk audit if nativesend is true then _amount should be equal to msg.value or other wise it will fail value becomes zero,
            //then go to the _isBuy condition and if its true then it will call placeAndExecuteMarketBuy function with value as zero but in the function amount to mutliply with price precision and divide by 10**_marketParams.quoteAssetDecimals even if its zero it will cause any issue?
            uint256 _value = _nativeSend[i] ? _amount : 0;
            _amountOut = _isBuy[i]//if its true then go for buy or if its false then go for sell
                ? (
                    //if nativesend is true then check the amount sent should be equal to msg.value or not
                     //if(_nativeSend[i]){
                    //  require(_amount == msg.value,RouterErrors.NativeAssetMismatch());
                    // }

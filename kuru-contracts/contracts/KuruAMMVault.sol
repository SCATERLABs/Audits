...
1. After checking other contracts,is there any isssue with fully approval to margin account?
2.Asymmetric / questionable math in _getVaultSizesForBaseAmount — Low/Informational — ask/bid sizing looks asymmetric; may be intentional but unclear.
3.
 */
...
contract KuruAMMVault is IKuruAMMVault, ERC20, Initializable, UUPSUpgradeable, ReentrancyGuardTransient {
    ...
    function initialize(
        ...
        //@nk-info why not checking owner address(0)?
        //@nk check spread constant >0 otherwise div by 0 error?
        owner = _owner;
        token1 = token1_;
        ...
        //@this is where if spread check is not checking then it will cause div by 0 error
        SPREAD_CONSTANT = _spreadConstant;
        //setMarketParams() calling but where the params data are stored?
        //in marketParams struct
    ...
    function transferOwnership(address _newOwner) external {
        _checkOwner();
        //@nk info if the newowner is zero then it will cause the no one will control
        owner = _newOwner;
    }
     ...
     */
        //@audit-issue :if both native tokens then leads to loss of funds
        function deposit(uint256 baseDeposit, uint256 quoteDeposit, address receiver)
            ...
            returns (uint256)
        {
            //@nk if both deposit with native token then what happens?
            //@nk if one native other one ERC20 token then what happens?
            //@check address of receiver !=0 
            //@audit require(msg.value == baseDeposit + quoteDeposit) this is current bug or not check it  and refunds are sent because some tokens are locked i think check it 
            //@check baseDeposit and quoteDeposit both are >=0
            require(
    ...
    function _mintAndDeposit(uint256 baseDeposit, uint256 quoteDeposit, address receiver) internal returns (uint256) {
        ...
        //@nk if bashDeposit is zero what happens?
        uint256 _shares;
        if (totalSupply() != 0) {//total supply means total shares or total assets 
            uint256 _expectedQuoteAmount = ( //if baseDeposit is zero then expected quote amount is also zero
                (
            ...
            // @check what if expectedquoteamount is less than quotedeposit this means quotedeposit store in the vault heavy it cause something
            //@nk check if expectedQuoteamount is satisified because its zero when the baseAmount is zero
            ...
            //@nk check if the msg.value is 2 ethers and basedeposit a,quoteDeposit b large value give then it will mint shares for a,b
            _mint(receiver, _shares);
            //@nk if get liquidate some amount 5 eth and 20000usdc use some values remaining eth not send to the user who will sent an amount
        } else {
            //@ if anyone first deposit in the vault loss some sort of liquidity(shares) ---ok
            //if any one of them becomes zero then what happens?
            // require(baseDeposit > 0 && quoteDeposit > 0, KuruAMMVaultErrors.InsufficientLiquidityMinted());

            _shares = FixedPointMathLib.sqrt(baseDeposit * quoteDeposit);
    //here also check _shares will be zero or not

            _mint(address(marginAccount), MIN_LIQUIDITY);
            _shares -= MIN_LIQUIDITY;
            _mint(receiver, _shares);
           //@nk check if both deposits are zero then what happens or one of them is zero then what happens?
            _currentAskPrice = (
        ...
        uint256 _nativeRefund =
            ...
            //@audit what if one is native token another one erc20 token
            //@audit naive refund is take care of native tokens to refund what if both are native tokens and what about the ERC20 tokens refund
        if (_nativeRefund > 0) {
    ...
    function _depositAmountsToMarginAccount(address _token1, address _token2, uint256 baseDeposit, uint256 quoteDeposit)
        internal
    {
        //@audit chekck require(msg.value == baseDeposit + quoteDeposit) is needed
    ...
    function _returnNormalizedAmountAndPrice(bool roundUp) internal view returns (uint256, uint256) {
        ...
        uint96 _normalizedBaseAmount = roundUp
            ? toU96(
                ...
                FixedPointMathLib.mulDivUp(
                    //@nk spread constant must be greater than zero
        ...
        uint256 _price = market.vaultBestAsk();//best price
        //give me example values here to understand better  
